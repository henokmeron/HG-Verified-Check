...
class OAuthEmailService {
  ...
  private async initializeTransporter() {
    try {
      console.log('🔧 Initializing OAuth email service...');
      ...
      console.log('🔄 Gmail refresh token available:', !!process.env.GMAIL_REFRESH_TOKEN);
      
      // Check if OAuth credentials are available
      if (!process.env.GMAIL_ADDRESS || !process.env.GMAIL_CLIENT_ID || !process.env.GMAIL_CLIENT_SECRET) {
        console.warn('⚠️ Gmail OAuth credentials not configured - email sending disabled');
        console.warn('Missing: ', {
          address: !process.env.GMAIL_ADDRESS ? 'GMAIL_ADDRESS' : 'OK',
          clientId: !process.env.GMAIL_CLIENT_ID ? 'GMAIL_CLIENT_ID' : 'OK',
          clientSecret: !process.env.GMAIL_CLIENT_SECRET ? 'GMAIL_CLIENT_SECRET' : 'OK'
        });
        return;
      }

      console.log('📬 Creating Gmail OAuth2 client...');
      ...
      if (process.env.GMAIL_REFRESH_TOKEN) {
        ...
        console.warn('⚠️ No refresh token available - need to authorize first');
        console.log('📝 To get refresh token, visit: http://localhost:3000/auth/google');
        return;
      }
      ...
      if (error.message?.includes('invalid_grant') || error.message?.includes('refresh_token')) {
        console.error('\n📝 Gmail OAuth Authentication Failed - Instructions:');
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.error('The refresh token is invalid or expired. You need to re-authorize.');
        console.error('\nTo get a new refresh token:');
        console.error('1. Visit: http://localhost:3000/auth/google');
        console.error('2. Sign in with your Google account');
        console.error('3. Grant permissions');
        console.error('4. Copy the refresh token from console');
        console.error('5. Update GMAIL_REFRESH_TOKEN environment variable');
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
      }
...
export const oauthEmailService = new OAuthEmailService();
